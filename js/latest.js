document.getElementById("butRefresh").addEventListener("click", function() {
      ...
      .then(function(response) {
        ...
        for (var i = 0; i < commitContainer.length; i++) {
          container.querySelector("" + commitContainer[i]).innerHTML =
            ...
            "</h4>" +
            "<h4> Time committed: " +
            new Date(response[i].commit.author.date).toUTCString() +
            "</h4>" +
            "<h4>" +
            "<a href='" +
            response[i].html_url +
  ...
  function fetchCommitsFromLocalStorage(data) {
    ...
    for (var i = 0; i < commitContainer.length; i++) {
      container.querySelector("" + commitContainer[i]).innerHTML =
        "<h4> Message: " +
        localData[posData[i]].message +
        "</h4>" +
        "<h4> Author: " +
        localData[posData[i]].author +
        "</h4>" +
        "<h4> Time committed: " +
        new Date(localData[posData[i]].time).toUTCString() +
        "</h4>" +
        "<h4>" +
        "<a href='" +
        localData[posData[i]].link +
        "'>Click me to see more!</a>" +
        "</h4>"
    }
  }
